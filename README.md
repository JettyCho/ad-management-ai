...
MCP가 아닌 CLI로 직접 동작하는 Skill 기반 도구들입니다.

| 스킬 | 방식 | 설명 |
|---|---|---|
| `slack-manage` | curl + Slack Web API | 메시지 전송, 채널 조회, 검색, 파일 업로드 (본인 이름으로 동작) |
| `slack-summary` | curl + Slack Web API | 슬랙 채널 대화 요약 및 분석 |
| `aws-manage` | aws-vault + AWS CLI | AWS 리소스 조회 및 관리 |
| `sentry-fix` | Sentry MCP + GitHub | Sentry 이슈 분석 및 수정 PR 생성 |
...
├── .claude/
│   ├── skills/            ← 커스텀 스킬 정의
│   │   ├── slack-manage/  ← Slack 메시지 전송, 검색 등
│   │   ├── slack-summary/ ← Slack 채널 대화 요약
│   │   ├── sentry-fix/    ← Sentry 이슈 분석 및 수정
│   │   └── aws-manage/    ← AWS 리소스 조회/관리
