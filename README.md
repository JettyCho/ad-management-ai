...
**Salesforce** (CLI 설치 + Org 인증 방식):

Salesforce는 별도 CLI를 설치하고 터미널에서 Org 인증을 완료해야 합니다.

1. Salesforce CLI를 설치합니다.
   ```bash
   npm install -g @salesforce/cli
   ```
2. Salesforce Org에 로그인합니다. 브라우저가 열리면 로그인 후 권한을 허용합니다.
   ```bash
   sf org login web --alias my-org --set-default
   ```
3. `/mcp`에서 salesforce 서버를 **재시작**합니다. (인증 전에 이미 연결된 경우, 재시작해야 org 정보를 인식합니다.)
