...
- **MySQL DB 이름**: `lockjoyUS` (Teleport에서 `prod-buzzad-replica-*`로 접속 시 실제 DB 이름은 `buzzad`가 아닌 **`lockjoyUS`**이다. DuckDB MySQL 확장이나 mysql CLI 사용 시 `database=lockjoyUS`로 지정해야 한다.)
...
- **그룹웨어** (https://gr.buzzvil.com) — 지출결의서, 문화생활비, 팀회식비 등 결제 업무를 처리하는 사이트
- **레몬베이스** (https://lemonbase.com) — 팀 OKR, 개인 성과 관리 등 성과 평가에 사용하는 사이트
- **플렉스** (https://flex.team) — 휴가 신청, 인사 업무 등을 처리하는 사이트
